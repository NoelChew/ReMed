...
public class DropboxSensorDataActivity extends AppCompatActivity {
    ...
    Runnable mStatusChecker = new Runnable() {
        ...
        public void run() {
            try {
                ...
                SensorDataApi.getSensorData(SensorDataApi.SensorType.UV, onGetUVSensorDataListener);
                SensorDataApi.getSensorData(SensorDataApi.SensorType.MOISTURE, onGetMoistureSensorDataListener);
            } finally {
    ...
    private OnGetSensorDataListener onGetTemperatureSensorDataListener = new OnGetSensorDataListener() {
        @Override
        public void onCallSuccess(final SensorDataSet sensorDataSet) {
...
//            HashMap<Integer, Integer> timestampHashMap = new HashMap<Integer, Integer>();
                    ...
                    if (newSensorValues == null || newSensorValues.isEmpty()) {
                        return;
                    }
...
//                lineChartTemperature.getXAxis().setEnabled(false);
    ...
    private OnGetSensorDataListener onGetUVSensorDataListener = new OnGetSensorDataListener() {
        @Override
        public void onCallSuccess(final SensorDataSet sensorDataSet) {
            runOnUiThread(new Runnable() {
                @Override
...
//            Log.d(TAG, "Last point location: " + convertToNearestThousands(emVideoView.getDuration()));
                    LineDataSet set1;
                    if (lineChartUV.getData() != null &&
                            lineChartUV.getData().getDataSetCount() > 0) {
                        set1 = (LineDataSet) lineChartUV.getData().getDataSetByIndex(0);
                        set1.setValues(values);
                        lineChartUV.getData().notifyDataChanged();
                        lineChartUV.notifyDataSetChanged();
                        lineChartUV.invalidate();
                    } else {
                        // create a dataset and give it a type
                        set1 = new LineDataSet(values, "DataSet 1");
...
//                    set1.setFormSize(15.f);
                        ...
                        // set data
                        lineChartUV.setData(data);

                        Legend l = lineChartUV.getLegend();
                        l.setEnabled(false);

//                lineChartTemperature.getAxisLeft().setEnabled(false);
//                lineChartTemperature.getAxisRight().setEnabled(false);
//                lineChartTemperature.getXAxis().setEnabled(false);
                        lineChartUV.setDescription("UV");

                        AxisValueFormatter xAxisFormatter = new HourAxisValueFormatter(reference_timestamp);
                        XAxis xAxis = lineChartUV.getXAxis();
                        xAxis.setValueFormatter(xAxisFormatter);
                        MyMarkerView myMarkerView = new MyMarkerView(getApplicationContext(), R.layout.custom_marker_view, reference_timestamp);
                        lineChartUV.setMarkerView(myMarkerView);
                    }
                }
            });
        }
    ...
    private OnGetSensorDataListener onGetMoistureSensorDataListener = new OnGetSensorDataListener() {
        @Override
        public void onCallSuccess(final SensorDataSet sensorDataSet) {
            runOnUiThread(new Runnable() {
                @Override
...
//            Log.d(TAG, "Last point location: " + convertToNearestThousands(emVideoView.getDuration()));
                    LineDataSet set1;
                    if (lineChartMoisture.getData() != null &&
                            lineChartMoisture.getData().getDataSetCount() > 0) {
                        set1 = (LineDataSet) lineChartMoisture.getData().getDataSetByIndex(0);
                        set1.setValues(values);
                        lineChartMoisture.getData().notifyDataChanged();
                        lineChartMoisture.notifyDataSetChanged();
                        lineChartMoisture.invalidate();
                    } else {
                        // create a dataset and give it a type
                        set1 = new LineDataSet(values, "DataSet 1");
...
//                    set1.setFormSize(15.f);
                        ...
                        // set data
                        lineChartMoisture.setData(data);

                        Legend l = lineChartMoisture.getLegend();
                        l.setEnabled(false);

//                lineChartTemperature.getAxisLeft().setEnabled(false);
//                lineChartTemperature.getAxisRight().setEnabled(false);
//                lineChartTemperature.getXAxis().setEnabled(false);
                        lineChartMoisture.setDescription("Moisture");

                        AxisValueFormatter xAxisFormatter = new HourAxisValueFormatter(reference_timestamp);
                        XAxis xAxis = lineChartTemperature.getXAxis();
                        xAxis.setValueFormatter(xAxisFormatter);
                        MyMarkerView myMarkerView = new MyMarkerView(getApplicationContext(), R.layout.custom_marker_view, reference_timestamp);
                        lineChartMoisture.setMarkerView(myMarkerView);
                    }
                }
            });
        }
